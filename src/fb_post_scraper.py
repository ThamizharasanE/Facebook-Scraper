...
# Install the appropriate version of ChromeDriver automatically
...
class Post_Scraper:
    ...
    def get_posts_info(self, soup, posts_urls_list=None, post_date_list=None, likes_list=None):
        """
        Extract the full story URL, the post reactions, and the publishing date.
        """
    ...
    def get_post_images(self, soup):
        """
        Extract the full story (post) images.
        Args:
            soup (`bs4.BeautifulSoup object`)
        Return:
    ...
    def get_post_description(self, soup):
        """
        Extract the post description (text).
        """
        p = soup.findAll("p")  
        if len(p) > 0:
            description_text =' ' 
    ...
    def get_post_reactions(self, soup):
        ...
        if reactions_url_tag is not None:  
            ...
            reactions_a_tag = reactions_soup.find_all('a', class_=re.compile('like_def.*'))
            for item in reactions_a_tag:
                reactions_img = item.find('img')
                if reactions_img is not None:
                    for reaction in self.REACTIONS_NAMES:
                        if reaction in reactions_img['alt']:
                            nbr_reactions[self.REACTIONS_NAMES.index(reaction)] = item.get_text().replace(reaction, '').replace('·', '')
        return nbr_reactions

    def more_comments(self, soup):
        """
        Get more comments by going to the next page if exists.
        
        Args:
            soup (`bs4.BeautifulSoup object`):
                Post HTML content.
        Return:
            A URL (`str`) to get the next page of comments.
        """
    ...
    def search_posts(self):
        """
        Search Facebook posts based on the search query.
        """
        self.driver.get(self.SEARCH_URL + self.search_query)
        time.sleep(5)
        ...
        while True:
            for a in soup.find_all('a', href=True):
                ...
                if 'story.php' in href or '/posts/' in href:
                    full_url = self.MBASIC_URL + href.replace('https://m.facebook.com', '')
                    posts_urls_list.append(full_url)
            more_posts_url = self.get_more_posts(soup)
            if more_posts_url:
                self.driver.get(more_posts_url)
                soup = BeautifulSoup(self.driver.page_source, 'html.parser')
            else:
                break
        return list(set(posts_urls_list))  # Return unique URLs

if __name__ == "__main__":
    search_query = input("Enter Search Query: ")
    scraper = Post_Scraper(search_query)
    scraper.login(username, password) 
    posts_urls_list = scraper.search_posts()
